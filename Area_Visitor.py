...
capture = cv2.VideoCapture("rtsp://admin:1234556@10.252.11.39/stream0")

path = ".\\image\\Area_Visitor"
...
while True:
    ...
    if nowMinute == "00" and nowSecond == "00":
        try:
            cv2.imwrite("image\\Area_Visitor\\Parking_Area_Visitor_" + str(now) + ".png", image)
        except:
            cv2.imwrite("image\\Area_Visitor\\Parking_Area_Visitor_" + str(now) + ".png", image)
